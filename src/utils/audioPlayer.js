 ...
 * @param {number} volume Volume level from 0.0 to 1.0 (defaults to 0.35 for backward compatibility).
 */
const playSound = async (fileName, soundEnabled = true, volume = 0.35) => {
  ...
  try {
    ...
    gainNode.gain.value = Math.max(0, Math.min(1, volume)); // Use provided volume, clamped 0-1
